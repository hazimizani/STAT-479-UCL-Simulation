...
for (r in 1:n_sims) {
  ...
  # Finals (single leg match)
  finals = data.frame(
    Team1 = sf_winners[1],
    Team2 = sf_winners[2]
  ) %>%
    dplyr::left_join(combined_lambda_hat %>% dplyr::rename(Team1 = team, Team1_ability = ability),
                     by = "Team1") %>%
    dplyr::left_join(combined_lambda_hat %>% dplyr::rename(Team2 = team, Team2_ability = ability),
                     by = "Team2") %>%
    dplyr::mutate(
  ...
  # Simulate single-leg final based on ability-derived probability
  final_outcome = rbinom(n = 1, size = 1, prob = finals$prob[1])
