...
all_combined = bind_rows(
  ...
  uefa_group_ro16 %>% mutate(Competition = "UCL")
)
...
sf_matchups = alt_results %>%
  ...
  pivot_longer(everything(), values_to = "Matchup") %>%
...
sf_plot = sf_matchups %>%
  ...
  labs(
    title = "Most Common Semifinal Matchups",
  ...
  theme(
    ...
    plot.margin = margin(10, 60, 10, 10)  # 👈 extra right margin
  )

sf_plot

ggsave(
  "ucl_semifinal_matchups.png",
  sf_plot,
...
final_matchups = alt_results %>%
  mutate(
    Final_Pair = paste(pmin(Finals_Hi, Finals_Lo),
...
final_plot = final_matchups %>%
  ...
  labs(
    title = "Most Common Final Matchups",
    subtitle = "2019–20 UEFA Champions League Simulation (Bradley–Terry + Poisson)",
    x = "Final Matchup",
    y = "Probability"
  ) +
